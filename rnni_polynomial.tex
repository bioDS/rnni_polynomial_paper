...
Under every such operation, the problem is reduced to searching a graph where vertices are trees and edges are given by pairs of trees connected by the rearrangement operation.
The most popular such operations are the nearest neighbour interchange, subtree prune and regrafting, and tree bisection and reconnection.
The problem of computing shortest paths and distances in each of these graphs, however, is $\np$-hard.
Designing practical algorithm for these problems is hence challenging.
For example, fixed-parameter tractable algorithms are known to compute distances under the subtree prune and regrafting operation.

Although ranked phylogenetic trees are one of the central objects of interest in applications, the complexity of computing tree rearrangement distances between these trees is still an open problem.
In this paper, we settle this problem for the ranked nearest interchange operation by establishing that the complexity depends on the weight difference between two types of tree rearrangement operations (rank moves and edge moves) and varies from quadratic to $\np$-hard.
...
We also propose a study of the parameter (weight difference between rearrangement operations) that changes the complexity of computing distances from quadratic to $\np$-hard.
