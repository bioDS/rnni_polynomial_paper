...
This problem can also be seen as a local search problem and hence a member of the class $\pls$ for any $\rho$.
Indeed, \textcite{Gavryushkin2018-ol} provide an algorithm that given two trees on $n$ leaves computes a path of quadratic in $n$ length between them.
Although it is of interest whether the notion of a neighbourhood of a path in $\rnni(\rho)$ can be introduced in such a way that the problem $\decprob{\rho}$ is $\pls$-complete for certain values of $\rho$, this question goes beyond the scope of this paper.

To prove $\np$-hardness it is standard to consider the decision version of the shortest path problem \autocite{Dasgupta2000-xa,Bordewich2005-nx}.
