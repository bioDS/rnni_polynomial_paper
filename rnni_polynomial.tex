...
Applied to these trees $\findpath$ would perform one $\rnni$ move in every \textbf{while} loop (line~\ref{alg:findpath:line:while_loop}).
Hence the length of the output path of $\findpath$ is quadratic in $n$.
, one $\rnni$ move is necessary, hence the length of the output path of $\findpath$ applied on these trees is quadratic.
...
We now show that the $\findpath$ algorithm relies on the fact that the rank move and the $\nni$ move have the same weight in the $\rnni$ graph, in an non-trivial way.
...
\todo{The rest of this paragraph should mimic the previous paragraph.}
Applied to these trees $\findpath$ computes a path of length three consisting of $\nni$ moves only.
However, there is a path of length $\rho + 2$ where first the two cherries of $T$ swap ranks, resulting in $[\{a_3,a_4\},\{a_1,a_2\},\{a_1,a_2,a_3,a_4\}]$, before the cherry $(a_1,a_2)$ resolves to $[\{a_3,a_4\},\{a_3,a_4,a_1\},\{a_1,a_2,a_3,a_4\}]$.
