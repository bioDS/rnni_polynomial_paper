...
Out of all possible pairs $T, R$ of such trees choose one with $\fp(T, R)$ as short as possible.

Denote the path $\fp(T,R)$ by $p$ and $\fp(T', R)$ by $p'$, and let $[v, w]$ be the interval in $T$ on which the $\rnni$ move connecting $T$ and $T'$ is performed.
\todo{make sure the notion of an interval is introduced somewhere}
Then the first move on $p$ must involve either the interval directly below $v$ or the interval directly above $w$, because otherwise the first move on $p$ and $p'$ coincide contradicting our assumption that $p$ is the shortest path violating inequality~(\ref{eqn:iff_inequality}).
