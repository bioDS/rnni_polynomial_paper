...
\begin{itemize}
\item Trees with the notion of an interval in a tree (if needed: edge vs rank intervals), clusters, clusters induced by nodes, subtrees induced by clusters, list representation of trees, discrete time-trees (for generalising the proof).
...
Note that $C_1$ can only intersect with either $A$ or $B$, hence the first of the following cases does not apply to $k = 1$.
For $k > 1$ we say $C_{k-1} = (R)_{k-1}$.
...
\todo{How do we know the algorithm doesn't have to consider $C_i$s for $i < k-1$? LC: the moves for building those cluster on $FP(T',R)$ are the same as on $FP(T,R)$, as they can't be done on intervals between $t-1$ and $t+2$. So these $C_i$ be clusters of $T$ and $T'$ already. $C_{k-1}$ can be different if the first move on $FP(T,R)$ is the first one considering $C_k$.}
