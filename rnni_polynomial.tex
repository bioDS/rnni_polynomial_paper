...
Suppress $C_i$ and $C_j$ in both $T'$ and $R$ to new leaves $c_i$ and $c_j$ (with rank zero) respectively and proceed as in Case $k = 1$.
...
The worst-case complexity of $\findpath$ is quadratic in the number of leaves, as there can be at most $n-2$ executions of the \textbf{for} loop (line~\ref{alg:findpath:line:for_loop}) and at iteration $k$ of the \textbf{for} loop there are at most $n-1-k$ iterations of the \textbf{while} loop (line~\ref{alg:findpath:line:while_loop}).
