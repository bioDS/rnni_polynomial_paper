...
\summary{Everything we know so far about the complexity of} $\rnni(\rho)$ \summary{-- $\rnni \in \p$ and $\nni \in \np$}
The graph $\rnni(0)$ ignores rank moves, as those are weighted by zero.
\todo{LC: Do we need a lemma proving this?}
...
As computing distances in $\nni$ is $\np$-complete, it is $\rnni(\rho) \in \np$ as well.
However, we will be able to show in Theorem~\ref{thm:rnni_polynomial} that $\rnni(1) \in \p$.
