...
\usepackage[ibidtracker=false,maxbibnames=9,minbibnames=9,maxcitenames=9,mincitenames=9,terseinits=true,backend=biber]{biblatex}
\usepackage{float}
\usepackage{graphicx}
...
\thanks{We thank Alexei Drummond, David Bryant, and Kieran Elmes for useful discussions about the weight difference between $\rnni$ moves, complexity, and applied aspects of our results.
Their comments improved our paper.}
\thanks{We acknowledge support from the Royal Society Te Ap\=arangi through a Rutherford Discovery Fellowship (RDF-UOO1702).
...
In this paper we provide an algorithm that efficiently computes shortest paths in the $\rnni$ space of ranked trees, which is based on biologically relevant tree rearrangement operations.
Hence the $\rnni$ graph of ranked trees combines the advantages of computationally tractable distance measures, such as Robinson-Foulds, with biological interpretability, similar to other rearrangement methods like $\nni$, $\spr$, and $\tbr$.
The efficient computation of $\rnni$ distances facilitates the inference of trees where the order of divergence events is relevant, for example in virus or cancer evolution.
$\rnni$ operations are more suitable for these applications than for example $\nni$ because of its computational efficiency, and also because trees that share an evolutionary hypothesis are closer to each other than to trees that do not share that hypothesis.
...
The existence of an efficient algorithm for computing distances between trees also motivates using the $\rnni$ space of ranked trees to build consensus or summary trees for summarising tree samples.
