...
To prove the Proposition we assume to the contrary, that is, there is a polynomial-time algorithm for computing shortest paths in $\rnni(\epsilon)$.
...
Note that there cannot be more than $r$ rank moves $m_r$ on $p_0'$ as we can assume that $p_0'$ is a shortest path in $\rnni(0)$.
However, $|p_\epsilon'| < |p_\epsilon|$ contradicts the fact that $p_\epsilon$ is a shortest path in $\rnni(\epsilon)$.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:complexity_above_rnni}]
