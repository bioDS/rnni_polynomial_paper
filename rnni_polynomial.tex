...
\proof
\todo{add what the diameter is}
Let $\delta = \frac1r$, where $r$ is the diameter of the graph $\rnni$.
...
\begin{proposition}
There exists $\delta > 0$ such that the shortest path problem can be solved in polynomial time with the $\findpath$ algorithm in $\rnni(1-\epsilon)$ for $\epsilon < \delta$.
\end{proposition}
...
We prove the theorem by contradiction and assume that there is an $\epsilon < \delta$ and a pair of trees $T,R$ in $\rnni(1-\epsilon)$ for which there is a path $p'$ in $\rnni(1-\epsilon)$ that is shorter than $\fp(T,R)$.
If $m_n$ is the number of $\nni$ moves on $p'$ and $m_r$ the number of rank moves, it is $|p'| = m_n + m_r \cdot (1 - \epsilon)$.
...
\todo{I don't think this is a contradiction because $p$ is a path in $\rnni(1)$ and $FP(T, R)$ is a path in $\rnni(1-\epsilon)$.}
