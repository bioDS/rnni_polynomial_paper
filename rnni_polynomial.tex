...
\summary{We now show that} $\rnni \in \mathcal P$.
\begin{theorem}
...
\begin{theorem}
	If two ranked trees $T$ and $R$ share a cluster $C$, then every tree on every shortest path in $\rnni$ contains this cluster $C$.
	\label{cluster_thm}
\end{theorem}

\proof
	Assume to the contrary that $T$, $R$, and $C$ is a courter-example to the statement.
	We can assume that $d(T, R)$ is the smallest possible so the first tree $T'$ after $T$ on the shortest path from $T$ to $R$ does not contain the cluster $C$.
	Therefore $T$ and $T'$ are connected by an $\nni$ move that moves a subtrees rooted in a children of $(C)_T$ out of the subtree induced by $C$ -- see Figure~\ref{fig:cluster_thm_proof}, where we assume that the clusters induced by the children of $(C)_T$ are $C_1$ and $C_2$.
	Note that the only cluster that is different in $T$ and $T'$ is $C$ (in $T$) and the cluster induced in $T$ by the of rank $(C)_T'$.
	...
	$T$ contains the cluster $C = C_1 \cup C_2$, $T'$ does not contain this cluster.
...
>>>>>>> fec03da878a3287a5c689b9813585a5c86fa3618
\endproof
