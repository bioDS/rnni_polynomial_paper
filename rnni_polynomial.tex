...
Some of the most popular methods inter trees from sequences via maximum likelihood \autocite{Stamatakis2006-xb, Guindon2010-lo}, MCMC \autocite{Ronquist2003-eq, Suchard2018-tw, Bouckaert2018-yr}, distance- and parsimony-based searchers \autocite{Tamura2011-ky}.
...
Furthermore, all nodes in ranked trees are assigned \emph{ranks} by a function $\rank$ that bijectively maps internal nodes onto elements of the set $\{1, \ldots, n-1\}$ such that children have strictly smaller rank than their parents and all leaves onto $0$.
For a ranked tree $T$ we refer to its internal node of rank $k \in [n-1]$ by $(T)_k$.
...
We then say that this internal node \emph{induces} the cluster $C$ and say that the subtree rooted in this node is \emph{induced} by $C$.
...
Note that there are $n-2$ non-trivial clusters in a tree on $n$ leaves, and for simplicity we will from now on refer to these whenever we say clusters.
