...
In the following we consider the $\nni$ move between $T$ and $T_1$ building the cluster $A \cup B \cup D$ and distinguish three cases, depending on whether $C_k$ contains, besides taxa of $D$, just taxa of $A$ or $B$, or both.
Note that for the case $k = 1$ it must be $C_k \subseteq A \cup D$ or $C_k \subseteq B \cup D$ and if $k > 1$ we assume $C_k = (R)_{k-1}$.
...
\begin{enumerate}[label = \theenumi.\arabic*]
    ...
    \item If $C_k \subseteq A \cup D$, the move on $T_1$ decreases the rank of $(C_k)_{T_1}$ by exchanging $B$ and $D$, resulting in the tree  $T_2$ as depicted in the the middle of the bottom row in Figure~\ref{fig:thm_fp_nni2a}.
