...
\summary{Historically, the complexity question was following hand in hand with the split property -- more history plus biological relevance of the property.}
...
\proof
    To prove the theorem we assume the contradictory, that is, there is a shortest path between $T$ and $R$ in $\rnni(0)$ that contains a tree that does not have $C$ as a cluster, even though both $T$ and $R$ contain $C$.
    We can assume that $T$ and $R$ build the pair with minimum distance that is connected by such a shortest path, meaning that the tree $T'$ following $T$ on this path does not have the cluster $C$.
    Therefore $T$ and $T'$ are connected by an $\nni$ move that moves one of the subtrees rooted in the children of $(C)_T$ out of the subtree induced by $C$.
    Such trees $T$ and $T'$ are depicted in Figure~\ref{fig:cluster_thm_proof}, where we assume that the clusters induced by the children of $(C)_T$ are $C_1$ and $C_2$.
    ...
    As $T$ and $R$ have minimum distance among all pairs of trees that share a cluster $C$, but are connected by a shortest path where not every tree contains this cluster, we can assume that the first move on $\fp(T',R)$ considers the cluster $C$.
    Similar to the argumentation in the beginning of the proof of Theorem~\ref{thm:rnni_polynomial}, all moves for clusters below $C$ in $R$ can be performed similarly on $\fp(T,R)$ and $\fp(T',R)$.
    Specifically, every move on $\fp(T,R)$ that changes the rank of the parent of the node inducing $C$ increases this rank and is followed by a move increasing the rank of node inducing $C$.
    ...
    Therefore, the first move on $\fp(T',R)$ builds the cluster $C$ and results in the tree $T$.
    It follows $|\fp(T,R)| \geq |\fp(T,R)|$, which contradicts our assumption that $T'$ is on a shortest path from $T$ to $R$.
...
\proof
    ...
    We construct trees $T'$ and $R'$ in $\rnni$ on $m+1$ leaves by adding $m + 1 - n$ leaves in the following way.
    ...
    The same way we receive tree $R'$ out of $R$.

    We can construct a path $p'$ from $T'$ to $R'$ out of any path $p$ from $T$ to $R$ where $|p| = |p'|$ by simply transforming length moves on $p$ to rank swaps on $p'$:
    If the rank $k$ of a cluster in a tree on $p$ changes to rank $k+1$ or $k-1$ by a length move, there is a rank swap induced by the node of these ranks in the corresponding tree on $p'$.
    All rank moves and $\nni$ move on $p$ can happen the same way on $p'$.
    By the construction of $T'$ and $R'$ it follows that $p'$ actually ends in $R'$, as there are no rank moves or $\nni$ moves needed inside the additional caterpillar subtree.
    And as every move on $p$ corresponds to exactly one move on $p'$, the lengths of these paths coincide.
    Note that the path we receive by transforming the moves on $\fp(T,R)$ into moves from $T'$ to $R'$ results in $\fp(T',R')$.

    With this it is not hard to see that $\fp(T,R)$ computes a shortest path.
    Indeed, if there was another path $p$ from $T$ to $R$ shorter than $\fp(T,R)$, we can get a path $p'$ with $|p'| = |p| < |\fp(T,R)| = |\fp(T',R')s|$ between $T'$ and $R'$ from $p$ as explained above, which contradicts the fact that $\fp(T',R')$ is a shortest path, as proven by Theorem~\ref{thm:rnni_polynomial}.
\endproof
