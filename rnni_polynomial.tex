...
This suggests a possibility of a positive threshold $\hat\rho \leq 1$ that separates the values of $\rho$ for which $\decprob{\rho}$ is polynomial from those for which $\decprob{\rho}$ is $\np$-hard.
In what follows, we provide infinitely many values of $\rho$ for which $\decprob{\rho}$ is not polynomial, leaving the question of the threshold existence open.

\begin{lemma}
Let $0 < \rho_0 \leq 1$.
If $\decprob{\rho} \in \fpc$ for all positive $\rho < \rho_0$ then ${\decprob{0} \in \fpc}$.
...
\proof
Let $T$ and $R$ be trees on $n$ leaves.
We set $\rho_1 = \min\{\frac{1}{n^2 + 1}, \frac{\rho_0}{2}\} < \rho_0$, and use the lemma assumption to compute a shortest path $p_1$ between $T$ and $R$ in $\rnni(\rho_1)$.
We will now show that the path $p_1$ is also a shortest path between $T$ and $R$ in $\rnni(0)$.

The following paragraph relies on the known \autocite{Gavryushkin2018-ol} fact that the diameter $\Delta(\rnni)$ of the $\rnni$ graph is at most $n^2$.
