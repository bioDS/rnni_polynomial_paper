...
% In general: Inferring ranked trees
...
Of particular interest in various applications is the order of internal nodes in these trees, as these nodes represent evolutionary events and their ranking shows the order in which these events happened in time.
For example in species evolution, where internal nodes of trees correspond to speciation events, the ranking of the tree gives the order of divergence events.
Often fossils are used to not only rank the tree, but assigning actual times to divergence events \autocite{Gavryushkina2014-xd}.
Other research fields where ranked trees play an important role are viral epidemiology, where ranking gives the order of transmissions \autocite{Ypma2013-zm}, and language evolution \autocite{Bouckaert2018-sf, Gray2009-jy}, where phylogenetic trees reveal how and when human populations expanded across different continents.
Nowadays phylogenetic trees are also commonly used in the research of cancer evolution \autocite{Potter2013-go, Jiao2014-ut}.
In cancer phylogenies internal nodes can refer to emergence of metastatic clones and their ranking shows in which order metastasis grew \autocite{Lote2017-hg}.

% Why we need distances
...
All these methods can be formulated as the problem of finding a tree optimising some criterion as a tree search problem.
For those problems a measure that tells how similar trees are is needed in order to compare trees.
Furthermore, summary or consensus tree methods \autocite{McMorris1994-no, Bansal2010-vr, Whidden2014-sx} require such a distance measure for trees.
Most of the currently used distance measures for trees, however, do not take the order of divergence events into account, but only the tree topology.
Moreover, popular tree distances are either hard to compute, or make little sense biologically \autocite{Whidden2018-fw}.

% Mathematical problem
% Why tree rearrangement operations
...
% Tree rearrangement distances are hard to compute -- some history
...
% Simpler distances such as Robinson Foulds lack biological motivation
Another popular tree distance measure that does not rely on a tree rearrangement method used in such methods is the Robinson-Foulds distance \autocite{Robinson1981-fb}.
In contrast to the tree rearrangement methods mentioned above, this distance can be computed efficiently. 
A downside of this approach however is that there is no biologically meaningful interpretation of this distance.
The Robinson-Foulds distances is not motivated by a biological process, unlike $\spr$, where a trees of distance one can be biologically connected by a hybridisation event \autocite{Bordewich2005-nx}.
...
% FP to solve the problem of computing distances (between ranked trees) efficiently
...
$\findpath$ furthermore reveals the following property of the $\rnni$ graph, which is desirable for tree distances from a biological point of view.
If two trees share some information, more specifically clusters, which we formally introduce later in the paper, there is a shortest paths in $\rnni$ that preserves this information.
More specifically, the shortest path computed by $\findpath$ maintains clusters, which is not true for $\nni$ \autocite{Li1996-zw}.
...
% We connect RNNI and NNI by introducing the parameter rho
...
This underlines that the graphs $\rnni(\rho)$ where rank moves are weighed by $\rho < 1$, and therefore less than $\nni$ moves, are relevant for biological applications.

% Link back to biology
The results of this paper show that for ranked trees there is an algorithm that computes distances efficiently and is biologically meaningful.
Hence the $\rnni$ graph of ranked trees combines the advantages of computationally tractable distance measures, such as Robinson-Foulds, with biological interpretability, similar to rearrangement methods like $\nni$, $\spr$, and $\tbr$.
With our algorithm we have a new way to compute distances between ranked trees, which facilitates the inference of trees where the order of divergence events is relevant, for example in virus or cancer evolution.
$\rnni$ operations more suitable for these applications than for example $\nni$ because of its computational efficiency, and because trees that share an evolutionary hypothesis are closer to each other than to trees that do not share that hypothesis.
For a cancer phylogeny this can be interpreted as two trees that support the emergence of a metastatic clone early in time are closer to each other than a phylogeny that does not support this hypothesis.
Efficiently computing distances between trees also motivates using the $\rnni$ space of ranked trees to build consensus or summary trees for summarising tree samples.
