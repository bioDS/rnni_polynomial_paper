...
Theorems~\ref{thm:rnni_polynomial}~and~\ref{thm:nni_hard} imply that the complexity of computing the ranked nearest neighbour interchange distance between trees depends on the weight of rank rearrangements and can change from quadratic, when rank moves are counted as one rearrangement, to $\np$-hard, when rank moves are ignored.
