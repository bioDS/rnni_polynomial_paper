...
\proof
    We assume that $\decprob{\rho} \in \p$ for all $\rho > 0$.
    Let $T$ and $R$ be trees in $\rnni(0)$, and let $n$ be the number of leaves of these trees.
    For proving the lemma we need to show that we can computes the distance between $T$ and $R$ in polynomial time.
    Therefore, we consider a shortest paths between $T$ and $R$ in $\rnni(\epsilon)$ with $\epsilon := \frac{1}{r+1}$, where $r = \Delta(\rnni)$ is the diameter of $\rnni$.
     We now show that a shortest path in $\rnni(\epsilon)$, which can be computes in polynomial time, gives us a shortest path in $\rnni(0)$.
    ...
    For contradiction we assume that there is a path $q_0$ in $\rnni(0)$ that is shorter than $p_0$.
    It follows $|q_0| \leq |p_0| - 1$.
    For the path $q_\epsilon$ in $\rnni(\epsilon)$ consisting of the same trees as $q_0$ it is $|q_\epsilon| = |q_0| + m_r \epsilon < |q_0| + 1 \leq |p_0| \leq |p_\epsilon|$, where $m_r$ is the number of rank moves on $q_0$.
...
The following is a direct result of Lemma~\ref{lemma:if_decprob_poly} and the fact that we cannot compute $\nni$ distances in polynomial time.
...
\summary{Finding a sequence of $\rho$s for which the problem is not in $\p$ and that converges to $0$.}

With the above we will be able to find a sequence of parameters $\rho$ for which $\decprob{\rho}$ is not solvable in polynomial time.
...
\todo{prove this thm -- the same as the thm above}
With this theorem and the knowledge that $\decprob{0} \notin \p$ we get the following result.

\begin{lemma}
    If $\decprob{\rho_0} \notin \p$, then exists a $\rho_1$ with $0 < \rho_1 < \rho_0$ such that ${\decprob{\rho_1} \notin \p}$.
    \label{lemma:more_rho_not_poly}
\end{lemma}
...
\proof
    This follows from Theorem~\ref{cor:exists_rho_not_poly} and iteratively applying Lemma~\ref{lemma:more_rho_not_poly}.
\endproof
