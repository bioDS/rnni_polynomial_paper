...
\proof
To show that $\findpath$ is a deterministic algorithm (see the pseudocode above), we have to prove that tree $T''$ constructed in the \textbf{while} loop (line~\ref{alg:findpath:line:while_loop}) of the algorithm always exists and is uniquely defined.
If $T''$ is obtained in line~\ref{alg:rank_step} from $T'$ by a rank move, the tree exists and is unique because there always exists exactly one rank move on any particular interval.
It remains to show that the $\nni$ move that decreases the rank of $(C_k)_{T'}$ always exists and is unique.
To prove this we consider cases $k = 1$ and $k > 1$ separately.

\begin{itemize}
\item[Case $k = 1$.]
In this case $C_k$ consists of two leaves $\{x, y\}$.
Since we assumed that the \textbf{while} condition is satisfied, the node $v = (\{x, y\})_{T'}$ has rank $r > 1$.
...
Now observe that out of two $\nni$ moves possible on the edge $[u, v]$ in $T'$, only the one that swaps $T'_2$ and $T'_3$ does decrease the rank of the most recent common ancestor of $\{x, y\}$.
Hence $T''$ exists and is unique in this case.
...
As in Case $k = 1$, exactly one of two possible $\nni$ moves deceases the rank of the most recent common ancestor of $c_i$, $c_j$ in $T'$, so the same is true for the most recent common ancestor $(C_k)_{T'}$, and $T''$ is unambiguously defined.
\end{itemize}
...
In this case, $C_{k-1} = A \cup B$ is a cluster in $R$.
...
In this case $C_k \subseteq A \cup B$,
\todo{Why is $C_k$ the first cluster to satisfy the \textbf{while} condition? Why not $k-1$ or even lower?}
so the first move on $\fp(T', R)$ decreases the rank of $(C_k)_{T'}$ by exchanging the subtrees induced by $B$ and $C$.
