...
This geodesic metric space is called $\mathrm t$-space and an algorithm for computing geodesics in $\mathrm t$-space would be of importance for applications \autocite{Gavryushkin2016-uu}.
\end{enumerate}


\section{Appendix}

\begin{proof}[Proof of Proposition~\ref{prop:complexity_around_nni}]
...
We prove this proposition by reducing the shortest path problem in $\rnni(0)$ to the shortest path problem in $\rnni(\epsilon)$.
Therefore, we assume that $T,R$ are two trees with a $\rnni(0)$ path $p$ between them.
Let path $p'$ be the path in $\rnni(\epsilon)$ containing the same trees as $p$.
We now show that $p$ is a shortest path if and only if $p'$ is a shortest path.

Let $p$ be a shortest path in $\rnni(0)$.
For contradiction we assume that there is a path $p''$ in $\rnni(\epsilon)$ that is shorter than $p'$.
%TODO What if $p''$ and $p'$ have the same number of $\nni$ moves but $p''$ has less rank moves?
If $p''$ consists of $m_n$ $\nni$ moves and $m_r$ rank moves, it is $|p''| = m_n + m_r \epsilon < |p'|$.
IDEA: We consider $p''$ in $\rnni(0)$ and want to show that this path is shorter than $p$ which leads to a contradiction.
\end{proof}
