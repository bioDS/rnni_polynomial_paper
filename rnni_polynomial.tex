...
The first moves on $p$ and $p'$ must involve the interval either directly below $v$ or directly above $w$, because otherwise these first moves must coincide contradicting our assumption that $p$ is the shortest path violating inequality~(\ref{eqn:iff_inequality}).

Denote the first tree after $T$ on $p$ by $\hat T$
\todo{do we need any of these indices apart from $k$ in what follows?}
