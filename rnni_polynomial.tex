...
Since $\rnni(\rho)$ is a connected graph with its diameter bounded by a polynomial in $n$ \autocite{Gavryushkin2018-ol}, the problem $\decprob{\rho}$ in general is in complexity class $\tfnp$ \autocite{Megiddo1991-lr}.

Our main goal is to prove that $\decprob{1}$ is polynomial.
...
Consider the following three disjoint subtrees of $T'$.
The subtree $T'_1$ induced by a child of $u$ and containing $x$.
The subtree $T'_2$ induced by the other child of $u$.
The subtree $T'_3$ induced by a child of $v$ and containing $y$.
...
\todo{This case requires more work}
Case $k > 1$.
\begin{enumerate}
...
\item $C_k = C_i \cup \{x\}$, where $x$ is a leaf not present in $C_1 \cup \ldots \cup C_{k-1}$.
Suppress $C_i$ in both $T'$ and $R$ to a new leaf $c_i$ and proceed as in Case $k = 1$.
\item $C_k = \{x, y\}$ where neither of $x$, $y$ is in $C_1 \cup \ldots \cup C_{k-1}$.
