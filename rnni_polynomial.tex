...
Hence the cluster $C_k$ that is considered first by $\findpath$ must contain elements from both $B$ and $C$ but not from $A$, and the rank of $(C_k)_R$ has to be at most $t$.
...
In this case, the cluster $C_k$ that is considered by $\findpath$ in the first step of computing $\fp(T, R)$ must intersect $D$ but not $C$.
...
We can assume that $k > 1$ as $C_1$ contains two elements and can therefore not intersect three disjoint sets.
Since $C_k$ is the first cluster considered by $\findpath$, the two clusters that make up $C_k$ in $R$ must be present in $T$.
\todo{This claim is incorrect.}
So $C_{k-1} = A \cup B$.
