...
We say that a graph on trees has the \emph{cluster property} if for two trees $T$ and $R$ sharing a cluster it follows that every tree on every shortest path between $T$ and $R$ also has this cluster.
...
\todo{same in $R$, identical caterpillar tree}
