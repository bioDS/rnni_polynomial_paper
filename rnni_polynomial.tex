...
Specifically, our algorithm scales to trees with more than thousands of leaves.

We also connect the problem of computing distances in our graph of ranked trees with the well-known version of this problem on unranked trees by introducing a parameter for weight difference between move types.
...
When formulated as a local search problem, the family attains such complexity classes as $\fpc$, $\pls$, and $\tfnp$.
...
\summary{More motication: Distances that we can compute in polyomial time are not well motivated biologically, all others are $\np$-hard}
Computing shortest paths and distances between trees is not only essential for tree inference.
Another example showing the importance of distance measures for trees is summarising trees \autocite{Bansal2010-vr, Whidden2014-sx}.
A popular tree distance measure used for this is the Robinson-Foulds distance \autocite{Robinson1981-fb}, as it can be computed in linear time.
The downside of this approach however is the lack of biological motivation.
In general, distance measures that are easy to compute have this problem, whereas measures that are biologically relevant are often hard to compute.
...
For computing rearrangement distances between trees, that is, the minimum number of tree rearrangements necessary to convert one tree into another, classical results in mathematical phylogenetics imply that this problem is $\np$-hard \autocite{Dasgupta2000-xa, Bordewich2005-nx, Hickey2008-wv, Allen2001-ky} for all three rearrangement operations $\nni$, $\spr$, and $\tbr$.
...
The shortest path problem $\decprob{\rho}$ is in the complexity class $\tfnp$\autocite{Megiddo1991-lr} for every $\rho$, as $\rnni$ is a connected graph with diameter bounded by a polynomial \autocite{Gavryushkin2018-ol}.
It can also be seen as a local search problem and member of the class $\pls$ for any $\rho$.
The question whether $\decprob{\rho}$ is $\pls$-complete for some values of $\rho$ however remains open.

To prove $\np$-hardness, the decision version of the shortest path problem is being considered in the literature \autocite{Dasgupta2000-xa,Bordewich2005-nx}.
The decision versio of $\decprob{\rho}$ can be seen as adding a parameter $k$ to the input of the problem and asking if a path of length less than or equal to $k$ between two given trees exists.
Whenever we say that $\decprob{\rho}$ is $\np$-hard or in $\p$, we refer to this corresponding decision problem.
...
The following corollary is a direct result of Theorem~\ref{thm:rnni_polynomial}.
