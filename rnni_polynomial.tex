...
We say that a graph on trees has the \emph{cluster property} if for all pairs of trees $T$ and $R$ sharing a cluster every tree on every shortest path between $T$ and $R$ also has this cluster.
...
\begin{enumerate}
\item Characterise the computational complexity of the shortest path problem in $\rnni(\rho)$ in terms of $\rho$.
