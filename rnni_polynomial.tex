...
\todo{define rank function or don't use it (in pseudo-code)}
...
\proof
To show that the algorithm terminates with a path ending in $R$, we need to show that in the \textbf{for} loop (line~\ref{alg:findpath:line:for_loop}) there is always exactly one $\rnni$ move possible to decrease $\rank((C_k)_{T'})$, until it is $k$.
...
The node $v = (\{x, y\})_{T'}$ has $\rank(v) = r > 1$.
Consider the node $w$ preceding $v$ in $T'$, that is, $\rank_(w) = r - 1$.
