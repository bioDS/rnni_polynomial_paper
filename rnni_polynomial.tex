...
For example in research fields like cancer and pathogen evolution, phylogenetic inference is used to gain information about evolutionary relationships \autocite{Beerenwinkel2015-cf, Hadfield2018-xp}.
Often not only the phylogenetic tree itself, but the timing of divergence events, which are displayed by internal nodes in the tree, is the centre of studies.
Time trees are for instance used for finding the time of a common ancestor of virus strains \autocite{Korber2000-sp} or analysing tumour and metastasis growth in cancer research \autocite{Lote2017-hg}.
...
Not only for tree inference, but also for summary or consensus tree methods \autocite{McMorris1994-no, Bansal2010-vr, Whidden2014-sx}, require such a distance measure for trees.
Most currently known distance measures for trees, however, do not take the order of divergence events into account, but only the tree topology.
Moreover, most popular tree distances are either hard to compute, or make little sense biologically \autocite{Whidden2018-fw}.
...
Lack of biological motivation however is a downside of this approach.
For example for summarising trees, using the Robinson-Foulds distance often results in poor summaries and hence is not used for summarising samples of trees obtained in a full Bayesian tree inference approach \autocite{Gavryushkin2016-uu}.
