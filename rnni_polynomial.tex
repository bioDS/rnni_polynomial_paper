...
Denote $\fp(T,R)$ by $p$ and $\fp(T', R)$ by $p'$, and let $[t, t+1]$ be the interval in $T$ on which the $\rnni$ move connecting $T$ and $T'$ is performed.
...
The first move on $p$ is on one of the intervals in $\{[t-1,t], [t,t+1], [t+1,t+2]\}$.
...
Denote the first tree after $T$ on $p$ by $\hat T$ and assume that the cluster representation of $R$ is
\todo{It's the first time $n$ appears, right?
Think if we want to be minimising over $n$.}
$[C_1, \ldots, C_{n-1}]$.
