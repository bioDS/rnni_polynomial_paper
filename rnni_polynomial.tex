...
The problem is similar in spirit to the ``beyond worst-case analysis'' \autocite{Roughgarden2019-to} because we have identified a parameter responsible for the shift of complexity from practically impossible, when the parameter is close to $0$, to highly efficient, when the parameter is $1$.
Indeed, a $\gamma$-perturbation stable instance of the maximum cut problem is known \autocite{Roughgarden2019-to} to be $\np$-hard for small values of $\gamma$ and polynomial for larger values of $\gamma$, and the problem of identifying the value of $\gamma$ where the complexity switches has largely been resolved \autocite{Makarychev2014-ev}.
We hence hope that the approaches reviewed by \textcite{Roughgarden2019-to} will be helpful for this study.
...
\proof
The trees $[\{a_1,a_2\},\{a_1,a_2,a_3\},\{a_1,a_2,a_3,a_4\}]$ and $[\{a_3,a_4\},\{a_2,a_3,a_4\},\{a_1,a_2,a_3,a_4\}]$ is the required counterexample.
\endproof


\section{Conclusion}

In this paper we have identified a class of shortest path problems $\decprob{\rho}$, $\rho \geq 0$, and showed that $\decprob{\rho}$ is $\np$-hard for $\rho$ close to $0$ and $\decprob{1}$ polynomial-time solvable.
As discussed in the introduction, the problem of characterising the range of $\rho$ where $\decprob{\rho}$ in polynomial is methodologically close to other similar problems, e.g.\ $\gamma$-perturbation stable instances of the maximum cut problem.
...
\item The same questions as we have considered for ranked $NNI$ can be studied in other rearrangement-based graphs on leaf-labelled trees, such as the ranked $\spr$ graph and the ranked $\tbr$ graph \autocite{Semple2003-nj}.
