...
Additionally, if $(T)_{t+2}$ is the parent of $(T)_{t+1}$, we denote the cluster induced by the child of $(T)_{t+2}$ that is not $(T)_{t+1}$ by $D$ -- see Figure~\ref{fig:thm_fp_nni1}.
...
\proof
Because two trees with the same tree topology but different rankings have distance $0$ in $\rnni(0)$, this graphs corresponds to a pseudo-metric space.
