...
The results of this paper show that for ranked trees there is an algorithm that does not only computes distances efficiently, but is also biologically meaningful.
Hence the $\rnni$ graph of ranked trees overcomes the computational issues of other tree rearrangement methods like $\nni$, $\spr$ and $\tbr$, but does not lack practical relevance like the Robinson-Foulds distance does.
With our algorithm we have a new way to compute distances between ranked trees, which facilitates the inference of trees where the order of divergence events matters, for example in virus or cancer evolution.
$\rnni$ operations are not only more suitable for these applications than for example $\nni$ because of its computational efficiency, but also because trees that share an evolutionary hypothesis are closer to each other than to trees that do not share that hypothesis. 
Being able to efficiently compute distances between trees also provides the opportunity to find new ways of building consensus or summary trees for summarising tree samples.
