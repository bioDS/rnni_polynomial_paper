...
\begin{itemize}
\item Trees (looks like we can get away without ``taxa'' -- CS folk understand leaves/ elements better) with the notion of an interval in a tree (if needed: edge vs rank intervals), clusters, clusters induced by nodes, subtrees induced by clusters, list representation of trees, discrete time-trees (for generalising the proof).
...
As we have shown that the complexity of computing the nearest neighbour interchange distance between ranked phylogenetic trees changes from NP-hard, when rank moves are ignored, to quadratic, when rank moves are counted as one rearrangement.
It hence is natural to ask what weight values of the two types of moves result in the problem becoming NP-hard.
In what follows we consider this question in more detail and leave its complete solution as an open problem.

Let $G_\rho$ be the $\rnni$ graph
\todo{We never mention $n$ anywhere. Shall we say it's fixed in the beginning? Otherwise we might want to add it in the decision problem}
where edges representing $\nni$ moves have weight $1$ and those representing rank moves have weight $\rho \in [0,1]$.
\todo{We might want to have this definition in the introduction?}
We consider the following decision problem:

\begin{decision_problem}[$\rnni(\rho)$]
Given $\rho \in [0,1]$, integers $k > 0$, and two ranked trees $T$ and $R$.
