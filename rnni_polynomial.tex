...
For proving the theorem we distinguish the two types of moves possible between $T'$ and $T$, that are $\nni$ or rank moves.

At first we consider the case that there is an $\nni$ move between $T'$ and $T$.
We assume that $T$ and $T'$ are the first trees on $p$ and $p'$, respectively, where a move is made that is changing one of the intervals incident to the edge $e = (v,w)$ (where $v$ is the node closer to the root) on which the $\nni$ move between $T$ and $T'$ is made.
Let us consider the different possible moves that effect $v$ or $w$ on $p$.
\begin{enumerate}
    \item $\nni$ moves on $e$
    ...
    As it is depicted in Figure~\ref{fig:thm_fp_nni1}, there are two $\nni$ moves on this edge.
