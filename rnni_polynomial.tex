...
\caption{Ranked trees in the $\rnni$ graphs with three $\nni$ moves on the left and a rank move on the right.
The two leftmost trees are caterpillar trees that only differ by the leaf labels $a_4$ and $a_5$.}
...
An \emph{interval} $[(T)_k,(T)_{k+1}]$ is defined by two nodes of consecutive ranks.
...
For example, the left-most tree in Figure~\ref{todo} have the following cluster representations.
...
Note that $(C)_T = (T)_k$ if the cluster $C$ is induced by the node of rank $k$ in $T$.
