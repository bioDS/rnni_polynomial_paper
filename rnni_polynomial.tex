...
We say that a graph on trees has the \emph{cluster property} if from two trees $T$ and $R$ sharing a cluster it follows that every tree on every shortest path between $T$ and $R$ also has this cluster.
...
We now show that Theorem~\ref{thm:nni_hard} can be extended to the values of $\rho$ close enough to $0$.
...
Assuming the threshold exists, the following proposition implies that $\hat\rho > 0$ and establishes an explicit lower bound on the threshold.
...
The following proposition implies that our algorithm $\findpath$ is not enough to isolate the values of $\rho$ for which $\decprob{\rho}$ is polynomial, and other algorithms have to be developed.
...
\item Can our results, e.g.\ the algorithm for computing shortest paths in $\rnni$, be used to establish the complexity of computing geodesics between trees with real-valued time intervals?
This geodesic metric space is called a $\mathrm t$-space and an algorithm for computing geodesics in $\mathrm t$-space would be of importance for applications \autocite{Gavryushkin2016-uu}.
\end{enumerate}

\subsection{Acknowledgements}

We thank Alexei Drummond and Kieran Elmes for useful discussions about the weight difference between $\rnni$ moves, applied aspects of our results, and the generalisation to discrete time-trees.
Their comments improved our paper.
We acknowledge support from the Royal Society of New Zealand through the Rutherford Discovery Fellowship (RDF-UOO1702) awarded to AG; Ministry of Business, Innovation, and Employment of New Zealand through the Endeavour Smart Ideas (CONT-61378-ENDSI-UOO) and Data Science Programmes grants.
