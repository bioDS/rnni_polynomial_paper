...
By adding a parameter $k$ the problem $\decprob{\rho}$ can be seen as a decision problem, asking if a path of length less than or equal to $k$ between two given trees exists.
...
\todo{Rename section? Before it was: For what $\rho$ is $\decprob{\rho}$ polynomial?}
...
\todo{do we want to keep this conjecture?  If talking about NP-hardness, it should be the decision version}
\begin{conjecture}
...
\begin{lemma}
	\todo{we could merge this and Lemma 2}
    If $\decprob{\rho} \in \fpc$ for all $\rho > 0$, then it is $\decprob{0} \in \fpc$.
    \label{lemma:if_decprob_poly}
\end{lemma}

\proof
	Let $T$ and $R$ be trees and let $n$ be the number of their leaves.
    By the assumption of the lemma, shortest paths between these trees can be computed in every space $\rnni(\rho)$ for $\rho > 0$ in polynomial time.
	Let $p_\epsilon$ be such a shortest path in $\rnni(\epsilon)$ with $\epsilon := \frac{1}{\Delta(\rnni)+1}$
	\todo{$\epsilon$ or $\rho_0$? -- $\rho_0$ results in awkward indexing: $p_{\rho_0}$}
	, where $\Delta(\rnni)$ is the diameter	of $\rnni$.
...
\todo{We cannot use $\decprob{\rho_0} \notin \p$ in the following statements. Shall we instead add a notation for the decision problem and use it here?}
\begin{corollary}
...
The following lemma is one step forward from Lemma~\ref{lemma:if_decprob_poly} towards constructing a sequence of values of $\rho$ for which $\decprob{\rho}$ is not polynomial.
...
\todo{Can we find a better place for this?}
...
%%% END: discussion of functional complexity classes
