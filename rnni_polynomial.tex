...
For the quadratic running time we assume the output of $\findpath$ to be a sequence of $\rnni$ moves rather than an actual sequence of trees.
Otherwise the running time increases to cubic as writing a tree takes time linear in $n$.
...
Note that we assume here that the output of $\findpath$ is not a sequence of trees, which would result in cubic running time, but a sequence of moves.
...
It follows that the worst-case size of the output of any algorithm for solving $\decprob{1}$ is quadratic.
\endproof
...
An illustration of these two paths can be found in Figure~\ref{fig:fp_rho_greater_1_counterexample}.
\begin{figure}[ht]
\centering
\includegraphics[width=0.6\textwidth]{fp_rho_greater_1_counterexample}
\caption{Comparison of a path computed by $\findpath$ (top) and an alternative path (bottom) that is shorter for $\rho > 1$.}
...
An illustration of these two paths can be found in Figure~\ref{fig:fp_rho_less_1_counterexample}.
\begin{figure}[ht]
\centering
