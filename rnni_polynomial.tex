...
Specifically, we consider trees $T'$ and $R'$ obtained from $T$ and $R$ by adding $m + 1 - n$ new leaves in the following way.
Add a new root of rank $m + 1$ to tree $T$ so that $T$ is a subtree rooted in one of the children of this new root.
...
Now, a path $p$ between $T$ and $R$ can be matched to a path $p'$ between $T'$ and $R'$ so that $|p| = |p'|$ by exchanging all length moves on $p$ to rank swaps on $p'$.
