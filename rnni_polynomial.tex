...
We know that all clusters $C_j$ with $j < k$ are present in $T$ already.
And since the first move on $\fp(T,R)$, decreasing the rank of $(C_k)_T$, happens on an interval bounded by ranks between $t$ and $t+2$, the most recent common ancestor of $C_k$ has rank less or equal to $t+1$ after this move.
Hence, it is $k \leq t + 1$.
It follows that all $C_j$ with $j < k - 1$ are present in $T'$ as well as in $T$, because all clusters induced by nodes of rank less than $t$ coincide in these two trees.
Cluster $C_{k-1}$, however, might not be induced by a node in $T'$ if $k-1 = t$.
Therefore, in the first move on $\fp(T',R)$ either the rank of the most recent common ancestor of $C_k$ or $C_{k-1}$ decreases.
