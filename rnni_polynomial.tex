...
\begin{theorem}
Distances in the $\rnni$ space can be computed in polynomial time.
\end{theorem}

\begin{proof}
We prove this theorem by showing that the algorithm $\findpath$ computes shortest paths in the $\rnni$ graph.
According to Lemma~\ref{lemma:iff_inequality}, it is sufficient to prove that for all trees $T,R$ and all neighbours $T' \in N_1(T)$ of $T$ it is $d_{FP}(T',R) \geq d_{FP}(T,R) - 1$ where $d_{FP}(T,R)$ is the length of the path from $T$ to $R$ computed by $\findpath$.
Lemma~\ref{lemma:inequality} proves that this inequality is true for all trees $T$ and $R$, which concludes the proof of this theorem.
\end{proof}
...
\begin{lemma}
For all trees $T$ and $R$ and all $T' \in N_1(T)$ it is $d_{FP}(T',R) \geq d_{FP}(T,R) - 1$.
