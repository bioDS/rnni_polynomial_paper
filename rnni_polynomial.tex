...
%wtf is wrong with this caption formatting?
...
We now consider all possible moves $\findpath$ can perform on interval $[(T)_t, (T)_{t+1}]$ to go from $T$ to $T_1$.

\begin{enumerate}[label = 1.{\arabic*}]
\todo{Wait.
That one is an edge everywhere in Case 1.
Got to go now...}
\item $\nni$ move, so $((T)_t, (T)_{t+1})$ is an edge in $T$.
