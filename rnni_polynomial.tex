...
Note that in all figures illustrating possible moves on $\fp(T,R)$ and $\fp(T',R)$ the position of the tree root is irrelevant, so we have positioned roots to simplify our figures.
...
In these cases, we will write ``$\rnni$ move" to mean a rank or an $\nni$ move, ``rank move" to mean a rank move, and ``$\nni$ move" to mean an $\nni$ move.
